...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  html += `<div class="header"><h1>${esc(name)}</h1><div class="meta">Analytical Show Report<br>${esc(now)}</div></div>`;
  ...
  html += `<div class="header"><h1>${esc(name)}</h1><div class="meta">Act Breakdown<br>${esc(now)}</div></div>`;
  ...
  for (let gi = 0; gi < a.actGroups.length; gi++) {
    ...
    html += `<div class="section-break"></div>`;
    html += `<div class="header"><h1>${esc(name)}</h1><div class="meta">Cue Listing<br>${esc(now)}</div></div>`;
